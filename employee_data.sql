...
alter table emp_data drop column `Company Tenure (In Months)`;
alter table emp_data drop column `Company Size`;


desc emp_data;

-- wirte sql to calculate employess count
select count(*)as emp_count from emp_data;

-- Describe the columns of emp_attrition_csv table
desc emp_data;

-- Write sql query to calculate average salary,average tenure and attrition rate  by job role
select `Job Role`,
round(avg(`Years at Company`),2) as avg_tenure,
round(avg(`Monthly Income`),2) as avg_salary,
(sum(case when Attrition='Left' then 1 else 0 end)*100/count(*))as attrition_percn
from emp_data group by `Job Role` order by avg_salary;

-- write sql query to calculate total salary ,average salary and max and min salary by Education Level
select `Job Role` , count(*)as total_emp,sum(`Monthly Income`)as total_salary,
round(avg(`Monthly Income`),2) as avg_salary,
max(`Monthly Income`) as max_salary,min(`Monthly Income`) as min_salary
from emp_data group by `Job Role`;

-- lets check is high scool education level candidates are hired in which job roles
select distinct(`Job Role`) from emp_data
where `Education Level`='High School';
...
from emp_data group by age order by Attrition_rate desc;
...
sum(case
 ...
 order by attri_percn desc;
 
 -- write sql query to calculate attrition rate by `Education Level`
 select `Education Level`,round(avg(`Monthly Income`),2) as avg_salary,
round(avg(`Years at Company`),2) as avg_tenure,
round((sum(case when Attrition='Left' then 1 else 0 end)*100/count(*)),2)as attrition_percn
from emp_data group by `Education Level` order by avg_salary;
 
 -- write sql query to calculate avergae salary by gender
 select Gender,round(avg(`Monthly Income`),2) as avg_salary
 from emp_data group by Gender order by avg_salary;
...
group by tenure_year order by attrition_rate desc; 
 ...
 from emp_data
 group by `Years at Company` order by `Years at Company`;
