...
group by tenure_year order by attrition_rate desc; 
 ...
 -- write sql query to know employee_count by education level in each job role
 
 select `Job Role`,`Education Level` ,count(*) as emp_count
 from emp_data group by `Job Role`, `Education Level` 
